                     ...
                     struct dentry *new_dentry)
{
	...
	/*Trace link operation */
	r = (struct record *)kmalloc(sizeof(struct record), GFP_KERNEL);
	...
	f_p = dentry_path_raw(old_dentry, tmp, 1024);
	...
	new_f_p = dentry_path_raw(new_dentry, tmp, 1024);
	r->new_path_length = strlen(new_f_p);
	r->new_path = kzalloc(r->new_path_length, GFP_KERNEL);
	if (r->new_path == NULL)
	{
		err = -ENOMEM;
		goto out;
	...
	memcpy(r->new_path, new_f_p, r->new_path_length);


	r->return_value = err;
	r->record_size = sizeof(r->id) + sizeof(r->type) +
	                 sizeof(r->return_value) +
	                 sizeof(r->file_address) +
	                 sizeof(r->path_length) + r->path_length +
	                 sizeof(r->new_path_length) + r->new_path_length;
	r->sb_private = (struct trfs_sb_info *)dir->i_sb->s_fs_info;
...
static int trfs_unlink(struct inode *dir, struct dentry *dentry)
{
	...
	f_p = dentry_path_raw(dentry, tmp, 1024);
	r->path_length = strlen(f_p);
	r->path = kzalloc(r->path_length, GFP_KERNEL);
	if (r->path == NULL)
	{
	...
	//memcpy(r->path, file->f_path.dentry->d_name.name, r->path_length);
	memcpy(r->path, f_p, r->path_length);


	r->return_value = err;
	r->record_size = sizeof(r->id) + sizeof(r->type) +
	                 sizeof(r->return_value) + sizeof(r->file_address)
	                 + sizeof(r->path_length) + r->path_length;
	r->sb_private = (struct trfs_sb_info *)dir->i_sb->s_fs_info;
	eworkqueue_init((void*)r);
...
static int trfs_mkdir(struct inode *dir, struct dentry *dentry,
                      umode_t mode)
{
	int err;
	struct dentry *lower_dentry;
...
static int trfs_mknod(struct inode *dir, struct dentry *dentry,
                      umode_t mode,
                      dev_t dev)
{
	int err;
                       ...
                       struct inode *new_dir, struct dentry *new_dentry)
{
	...
	r->record_size = sizeof(r->id) + sizeof(r->type) +
	                 ...
	                 sizeof(r->path_length) + r->path_length +
	                 sizeof(r->new_path_length) + r->new_path_length;
	r->sb_private = (struct trfs_sb_info *)old_dir->i_sb->s_fs_info;
	eworkqueue_init((void*)r);
...
static int trfs_readlink(struct dentry *dentry,
...
static const char *trfs_get_link(struct dentry *dentry,
                                 struct inode *inode,
